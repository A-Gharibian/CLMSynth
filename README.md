...
- **`mdcgen`**, fully synthetic geometries via `mdcgenpy` [[3]](#references), seeded for reproducibility.
...
- Every column other than `cluster_column` becomes a feature unless `tag_columns` names it
  (original names are kept, so plot axes show *your* names). Features must be numeric;
   a non-numeric column that is not declared a tag **rejects the file** rather than being dropped quietly.
- **`tag_columns`** lists columns that travel with the data without entering the CLM machinery: an outcome variable,
  a second labeling from another method, an identifier. They are copied to the output CSV unchanged, are excluded from
  the geometry and from `standardize`, and need not be numeric. A name that is not in the file is an error,
  and the cluster column cannot be one.
- Exactly one `cluster_column` is required, the run is **rejected** (logged, dataset skipped) if it is missing or names
...
- **`scope: "pair"` (`type: mcc`, `single` mode only)**, targets the `2×2` MCC
  ...
  exact solve a: the label is sized to sit entirely
...
> `R_K` [[2]](#references) that `clustering_mcc` computes (permutation-invariant via Hungarian
...
- **Target metric can be unreachable (structural ceiling).** MCC/ARI between an
  ...
  non-convergence warning. The engine does not yet compute or report the
  ceiling itself. *(planned for 0.8.0)*
...
- **The target-metric search grid cannot see inside a narrow feasible band.**
  ...
  true feasibility boundary is never explored. Refinement *within* the bracket is
  exact as of 0.7.0, which is a different thing: the bracket itself still comes
  from the grid. *(planned for 0.8.0)*
...
2. Gorodkin, J. (2004). Comparing two K-category assignments by a K-category
   ...
   367–374.
3. Iglesias, F., Zseby, T., Ferreira, D., et al. (2019). MDCGen:
